...
import static me.srrapero720.videolan4j.discovery.DiscoveryEnvironment.*;
...
public class NativeDiscovery {
    ...
    public static boolean discovery() {
        ...
        for (DiscoveryEnvironment environment: DiscoveryEnvironment.getStrategies()) {
            ...
            for (DiscoveryProvider p: getProviders()) {
            ...
            if (environment.onFound(provider, directory) && testInstance()) {
                ...
                discovered = true;
                LOGGER.info(IT, "Founded VLC {} on '{}', using '{}/{}'", VideoLan4J.getVideoLanVersion(), directory, environment.name(), provider.name());
                return true;
            } else {
                LOGGER.error(IT, "Failed loading VLC in '{}' using '{}/{}' cleaning JNA paths and trying again...", directory, environment.name(), provider.name());
    ...
    private static boolean testInstance() {
        try {
            ...
            LOGGER.error(IT, "Failed to attempt load VLC instance", e);
        }
        return false;
    }
    ...
    public static boolean testCleanup() {
        try {
            ...
            LOGGER.error(IT, "Failed to attempt clean broken discovered path", e);
        }
        return false;
    }
}
